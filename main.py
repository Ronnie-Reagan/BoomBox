...
import yt_dlp
import threading
from tkinter import Tk, Label, Entry, Button, Listbox, HORIZONTAL, Scale, Frame, Text, END, filedialog
...
# Music Directory
Label(settings_frame, text="Music Directory:", **LABEL_STYLE).pack(anchor="w", padx=10, pady=(10, 0))
music_dir_entry = Entry(settings_frame, width=60, **ENTRY_STYLE)
music_dir_entry.pack(padx=10, pady=2)
music_dir_entry.insert(0, MUSIC_FOLDER)
Button(settings_frame, text="Browse", command=lambda: browse_folder(music_dir_entry), **DARK_STYLE).pack(padx=10, pady=(0, 5))

# Video Directory
VIDEO_FOLDER = os.path.join(os.path.expanduser("~"), "Videos")
os.makedirs(VIDEO_FOLDER, exist_ok=True)

Label(settings_frame, text="Video Directory:", **LABEL_STYLE).pack(anchor="w", padx=10, pady=(10, 0))
video_dir_entry = Entry(settings_frame, width=60, **ENTRY_STYLE)
video_dir_entry.pack(padx=10, pady=2)
video_dir_entry.insert(0, VIDEO_FOLDER)
Button(settings_frame, text="Browse", command=lambda: browse_folder(video_dir_entry), **DARK_STYLE).pack(padx=10, pady=(0, 5))
